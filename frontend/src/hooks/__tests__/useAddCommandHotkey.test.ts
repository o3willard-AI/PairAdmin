...
import {
  useAddCommandHotkey,
  DEFAULT_ADD_COMMAND_HOTKEY,
} from "@/hooks/useAddCommandHotkey";
...
describe("useAddCommandHotkey", () => {
  beforeEach(() => {
    ...
    useCommandStore.setState({ commands: [], addCommandDialogOpen: false });
  ...
  it("opens the Add Command dialog when the default combo fires with the terminal focused", async () => {
    const { textarea } = fakeTermWithTextarea();
    useTerminalStore.getState().setTermRef("tab-1", { textarea } as unknown as Terminal);
    textarea.focus();

    const { unmount } = renderHook(() => useAddCommandHotkey());
    await flushMicrotasks();

    expect(useCommandStore.getState().addCommandDialogOpen).toBe(false);

    dispatchKeydown({ key: "P", ctrlKey: true, shiftKey: true });
    await flushMicrotasks();

    expect(useCommandStore.getState().addCommandDialogOpen).toBe(true);
    unmount();
  });

  it("uses a hotkey combo loaded from settings instead of the default", async () => {
    getSettings.mockResolvedValue({ HotkeyAddCommand: "Ctrl+Alt+Z" });
    const { textarea } = fakeTermWithTextarea();
    useTerminalStore.getState().setTermRef("tab-1", { textarea } as unknown as Terminal);
    textarea.focus();
  ...
  it("exports the documented default combo", () => {
    expect(DEFAULT_ADD_COMMAND_HOTKEY).toBe("Ctrl+Shift+P");
  });
});
